...
- **`query_logs` 新增 `by_logger` 全量聚合**（`aiops-datasource-mcp-server`）
  - **来自哪段代码**——"失败模式"最便宜也最稳定的近似。实测能把「业务代码抛的」
    与「容器/Servlet 包装的」分开
  - ⚠️ 描述里写明它**只是近似**：同一个失败会因容器包装被劈成两组（两条的 `stack_trace`
    首行相同），**归并请调用方自己判断**，不冒充精确的模式划分
  - 为什么不做真正的失败模式分组：terms agg **只能对已归一化字段**做，而失败模式要从
    自由文本提取——ingest pipeline 要改数据入湖链路（不在控制内）、runtime field 每次
    现算且脚本耦合、服务端侧归并只能对**取回的那一页**做（正是 P0 刚修掉的页内分布坑）
  - 描述里补了一条判据：**条数 ≠ 失败次数**（一次失败常写多条日志），要报影响面请按
    `logs[].trace_id` 去重
